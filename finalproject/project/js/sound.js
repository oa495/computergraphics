...
try {
    ...
    $('#song-info').text('Web Audio API is not supported in this browser');
}
...
request.onload = function() {
    context.decodeAudioData(
        ...
        function(buffer) {
            if(!buffer) {
                $('#song-info').text('Error decoding file data');
                return;
            }
            ...
            $('#song-info')
                .fadeOut('normal', function() {
                    $(this).html('<div id="artist">Beyonce, Hold Up.</div></div>');
                })
                .fadeIn();
        ...
        function(error) {
            $('#song-info').text('Decoding error:' + error);
        }
    );
};

request.onerror = function() {
    $('#song-info').text('buffer: XHR error');
};
